...
export class BoardService {
  ...
  private readonly _messageSubject: Subject<SubjectMessage> =
    new Subject<SubjectMessage>();
  ...
  constructor(configResolverService: ConfigResolverService) {
    ...
    this._config = {
      ...
      serializer: (message: SubjectMessage) => {
        const boardMessageBytes = Array.from(
          new Int8Array(ArrayUtilsService.boardMessageToBuffer(message.message))
        );
        // tslint:disable-next-line:no-bitwise
        return new Uint8Array(
          [message.subject & 0xff].concat(boardMessageBytes)
        );
      },
      deserializer: (event: MessageEvent) => {
        return new SubjectMessage(
          new Uint8Array(event.data.slice(0, 1))[0],
  ...
  public connect(): void {
    ...
    this._webSocketSubject.subscribe(
      (message) => this._messageSubject.next(message),
  ...
  public reconnect(): void {
    ...
    interval(this._reconnectTimeoutMs)
      .pipe(
        takeUntil(
          this._status.pipe(
            filter((status) => status === BoardServiceStatus.Online)
          )
        ),
        take(this._reconnectAttempts)
      )
      .subscribe(() => {
        this.connect();
      });
...
class SubjectMessage {
  constructor(
    ...
    public readonly message: BoardMessage
  ) {}
}
