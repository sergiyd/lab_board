...
export class ArrayUtilsService {
	...
	public static bufferToBoardMessage(buffer: ArrayBuffer): BoardMessage {
		...
		return new BoardMessage(new Uint8Array(buffer.slice(0, 2)).reduce((r, c, i) => r += c << (8 * i), 0),
			buffer.slice(2));
	}

	public static boardMessageToBuffer(boardMessage: BoardMessage): ArrayBuffer {
		// tslint:disable-next-line: no-bitwise
		const commandBytes = Array.from(new Uint8Array([boardMessage.command & 0xFF, boardMessage.command >> 8]));
		return new Uint8Array(boardMessage.data ?
