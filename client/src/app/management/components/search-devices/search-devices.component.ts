...
export class SearchDevicesComponent {
  public bus: DeviceBus = DeviceDecoratorService.buses.filter(
    (b) => b !== DeviceBus.Unknown
  )[0];
  ...
  public populate(device: ManagementSearchDevice): void {
    this._registerDeviceService.populate(
      `${device.typeName} device #${device.index + 1}`,
      this.bus,
      device.type,
