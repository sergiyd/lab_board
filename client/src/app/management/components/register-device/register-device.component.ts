...
export class RegisterDeviceComponent implements OnInit, OnDestroy {
  private static readonly buses: ReadonlyMap<DeviceBus, string> = new Map<
    DeviceBus,
    string
  >(
    RegisterDeviceService.buses.map((b) => [
      b,
      DeviceDecoratorService.busNames.get(b),
    ])
  );
  public static readonly busItems: readonly DeviceBusMenuItem[] = Array.from(
    RegisterDeviceComponent.buses.keys()
  ).map((k) => new DeviceBusMenuItem(k, RegisterDeviceComponent.buses.get(k)));

  private static readonly types: ReadonlyMap<DeviceType, string> = new Map<
    DeviceType,
    string
  >(
    Array.from(DeviceDecoratorService.types).map((t) => [
      t,
      DeviceDecoratorService.typeNames.get(t),
    ])
  );
  private static typesItems: readonly DeviceTypeMenuItem[] = Array.from(
    RegisterDeviceComponent.types.keys()
  ).map((k) => new DeviceTypeMenuItem(k, RegisterDeviceComponent.types.get(k)));
  ...
  public get address(): string {
    return ArrayUtilsService.addressToHex(
      Array.from(new Uint8Array(this._registerDeviceService.address))
    );
  }
