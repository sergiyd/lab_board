...
export class ManagementConsoleService extends BoardMessagingBaseService {
  ...
  private readonly _registeredDevicesSubject = new BehaviorSubject<
    Array<ManagementRegisteredDevice>
  >([]);
  private readonly _searchDevicesSubject = new BehaviorSubject<
    Array<ManagementSearchDevice>
  >([]);
  private readonly _registeredDevicesSubject$ = defer(() => {
    ...
    return this._registeredDevicesSubject as Observable<
      readonly ManagementRegisteredDevice[]
    >;
  }).pipe(
    finalize(() => {
  ...
  private sendRegisteredDevice(index: number): void {
    this.sendCommand(
      ManagementCommands.RegisteredDevice,
  ...
  private sendUnregisterDevice(index: number): void {
    this.sendCommand(
      ManagementCommands.UnregisterDevice,
      new Uint8Array([index])
    );
  }
  ...
  private sendAllDevicesCount(bus: DeviceBus): void {
    this.sendCommand(
      ManagementCommands.AllDevicesCount,
      new Uint8Array([bus.valueOf()])
    );
  }

  private sendDevice(bus: DeviceBus, index: number): void {
    this.sendCommand(
      ManagementCommands.Device,
      new Uint8Array([bus.valueOf(), index])
    );
  }

  private sendSwitchDevice(index: number, value: boolean): void {
    this.sendCommand(
      ManagementCommands.SwitchDevice,
      new Uint8Array([index, value ? 1 : 0])
    );
  }
  ...
  private sendRegisterDevice(device: Device): void {
    ...
    this.sendCommand(
      ManagementCommands.RegisterDevice,
      new Uint8Array(deviceData)
    );
  }
  ...
  private processRegisteredDevice(messageData: ArrayBuffer): void {
    ...
    const device = DeviceDecoratorService.decorateDevice(
      index,
    ...
    this._registeredDevicesSubject.value[index] =
      new ManagementRegisteredDevice(index, device);
  }
  ...
  private processSwitchDevice(messageData: ArrayBuffer): void {
    ...
    this._registeredDevicesSubject.value[index] =
      new ManagementRegisteredDevice(
        index,
        DeviceDecoratorService.setEnabledFlag(device.device, isEnabled)
      );
  }
  ...
  private processDevice(messageData: ArrayBuffer): void {
    ...
    this._searchDevicesSubject.value[index] = new ManagementSearchDevice(
      device.index,
      device
    );
  }
  ...
  public get registeredDevices$(): Observable<
    readonly ManagementRegisteredDevice[]
  > {
