...
export class DeviceDecoratorService {
  public static readonly buses: readonly DeviceBus[] = Object.keys(DeviceBus)
    .map((k) => Number(k))
    .filter((v) => !isNaN(v));
  ...
  public static readonly types: readonly DeviceType[] = Object.keys(DeviceType)
    .map((k) => Number(k))
    .filter((v) => !isNaN(v));
  ...
  public static getDeviceFlags(device: Device): number {
    // tslint:disable-next-line: no-bitwise
    return (
      (device.system ? DeviceFlags.System : 0) |
      (device.direction === DeviceDirection.Output
        ? DeviceFlags.Direction
        : 0) |
