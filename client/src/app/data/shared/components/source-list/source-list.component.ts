...
export class SourceListComponent implements OnInit, OnDestroy {
  public readonly formatters: readonly FormatterItem[] = Array.from(
    DataConvertorService.getFormatters()
  ).map((item) => new FormatterItem(item[0], item[1]));
  public readonly convertors: readonly ConvertorItem[] = Array.from(
    DataConvertorService.getConvertors()
  ).map((item) => new ConvertorItem(item[0], item[1]));
...
class FormatterItem {
  constructor(
    public readonly code: FormatterCodes,
