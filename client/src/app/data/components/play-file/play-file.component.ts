...
export class PlayFileComponent implements OnInit, OnDestroy {
  private readonly _chartDatasetsSubject = new BehaviorSubject<
      ...
      .subscribe((event) => {
        switch (event.type) {
          case HttpEventType.DownloadProgress:
            this._downloadProgressSubject.next(
              new DownloadProgress(event.loaded, event.total)
            );
            return;
          ...
          case PlayerAction.PlayBackward:
            interval(this.chartStepMs)
              .pipe(
                ...
                takeUntil(this._playerActionSubject)
              )
  ...
  private setData(fileData: FileData): void {
    ...
    this._chartDataMatix = new Map<number, Array<readonly number[]>>(
      this._fileData.devices.map((device) => [
        device.index,
        new Array<readonly number[]>(chartDeepMs / this.chartStepMs),
      ])
    );
    ...
    const datasets = this._fileData.devices.map((device) => {
      ...
      return new Dataset(
        source,
        this._sourceData$.pipe(
          map((sourcesSync) =>
            sourcesSync.data.find((data) => data.index === source.index)
          )
        )
      );
    });
  ...
  public set position(rawPosition: number) {
    ...
    // Clear values
    Array.from(this._chartDataMatix.keys()).forEach((deviceIndex) =>
      this._chartDataMatix.get(deviceIndex).fill(undefined)
    );
    ...
    for (const record of this._fileData.data) {
      ...
      if (recordTime > this._position) {
        ...
        matrixIndex =
          matrixMaxIndex - (this._position - recordTime) / this.chartStepMs;

        if (Math.abs(recordTime - this._position) < this.chartStepMs) {
          this._sourceDataSubject.next(
            new SourcesSync(
              record.t,
              record.d.map(
                (d) => new SourceData(d.i, d.f, new Uint8Array(d.v).buffer)
              )
            )
          );
        }
        record.d.forEach(
          (d) =>
            (this._chartDataMatix.get(d.i)[
              Math.floor(Math.max(0, matrixIndex))
            ] = d.v)
        );
      }
    }

    const chartDatasets = this._datasetsSubject.value
      .filter((dataset) => dataset.unmuted)
      .map((dataset) => {
        return {
          label: dataset.name,
          data: this._chartDataMatix.get(dataset.index).map((values) => {
            const dataView =
              values && new DataView(new Uint8Array(values).buffer);
            return DataConvertorService.convertNumber(
              dataView,
              dataset.convertionType,
              dataset.formatterCode
            );
          }),
          fill: false,
...
class DownloadProgress {
  ...
  constructor(public readonly loaded: number, public readonly total?: number) {
    if (total) {
