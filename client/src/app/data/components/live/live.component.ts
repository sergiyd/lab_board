...
export class LiveComponent implements OnInit, OnDestroy {
	...
	private _instantSyncIntervalSubject = new BehaviorSubject<number | undefined>(undefined);

	constructor(private readonly _liveDataService: LiveDataService) { }

	ngOnInit() {
		this._lastTimestamp = 0;
		this._liveDataService.fetching$.subscribe(value => this._fetching = value);

		this._datasets$ = this._liveDataService
			.sources$
			.pipe(takeUntil(this._unsubscribe))
			.pipe(mergeMap(sources => {
				this._viewDatasets.clear();

				return combineLatest(sources.map(source => source.pipe(map(s => new Dataset(s, this.initSourceDataSubject(s.index))), tap(dataset => {
					if (this._viewDatasets.has(dataset.index)) {
						this._viewDatasets.get(dataset.index).setDataset(dataset);
					} else {
						this._viewDatasets.set(dataset.index, ViewDataset.create(dataset));
		...
		this._liveDataService.sourcesSync$.pipe(takeUntil(this._unsubscribe)).subscribe(sync => {
			const timestamp = sync.timestamp;

			sync
				.data
				.filter(sourceData => this._sourcesDataSubject.has(sourceData.index))
				.forEach(sourceData => {
			...
			if (timestamp) {
				this._instantSyncIntervalSubject.next(timestamp - this._lastTimestamp);
	...
	private fetch(): void {
		...
		this._updateChartSubscription = interval(chartUpdatePeriodMs)
			.subscribe(() => this._chartDatasetsSubject.next(this.chartDatasets));
...
class ViewDataset {
	...
	public setDataset(dataset: Dataset): void {
		...
		this._sourceDataSubscription = this._dataset.sourceData$.subscribe(sourceData => {
