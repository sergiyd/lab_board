...
abstract class ConvertorBase implements Convertor {
  constructor(
    public readonly type: ConvertionTypes,
...
abstract class Acs712AConvertorBase extends ConvertorBase {
  ...
  public convert(data: DataView): number {
    ...
    return (
      (data.getInt16(0, true) - vValue * 2.5) / (vValue * this.coefficient)
    );
  }
}
...
abstract class FormatterBase implements Formatter {
  ...
  public format(value: number): string {
    return `${this.prefix || ""}${this.formatString(this.formatNumber(value))}${
      this.suffix || ""
    }`;
  }
...
class HexadecimalFormatter extends FormatterBase {
  constructor() {
    super(
      FormatterCodes.Hexadecimal,
      PhysicalClassifier.Number,
      "Hexadecimal",
      "0x"
    );
  }
...
class CelsiusFormatter extends FormatterBase {
  constructor() {
    super(
      FormatterCodes.Celsius,
      PhysicalClassifier.Temperature,
      "Celsius",
      "",
      "°C"
    );
...
export class DataConvertorService {
  ...
  private static readonly formatters: ReadonlyMap<
    FormatterCodes,
    FormatterBase
  > = new Map<FormatterCodes, FormatterBase>([
  ...
  private static readonly convertors: ReadonlyMap<
    ConvertionTypes,
    ConvertorBase
  > = new Map<ConvertionTypes, ConvertorBase>([
  ...
  public static getFormatters(): ReadonlyMap<FormatterCodes, string> {
    return new Map(
      Array.from(DataConvertorService.formatters.values()).map((formatter) => [
        formatter.code,
        formatter.name,
      ])
    );
  ...
  public static getConvertors(): ReadonlyMap<ConvertionTypes, string> {
    return new Map(
      Array.from(DataConvertorService.convertors.values()).map((convertor) => [
        convertor.type,
        convertor.name,
      ])
    );
  ...
  public static convertNumber(
    data: DataView,
    convertionType: ConvertionTypes,
    formatterCode: FormatterCodes
  ): number {
    if (!data) {
      return;
    }

    const convertor = DataConvertorService.convertors.get(
      convertionType || DataConvertorService.defaultConvertorType
    );
    const formatter = DataConvertorService.formatters.get(
      formatterCode || DataConvertorService.defaultFormatterCode
    );
