...
export class FileDataService extends BoardMessagingBaseService {
  ...
  private sendRemoveFile(name: string): void {
    this.sendCommand(
      HistoryCommands.RemoveFile,
      new Uint8Array(ArrayUtilsService.stringToArray(name))
    );
  }
  ...
  private processFilesCount(messageData: ArrayBuffer): void {
    ...
    this._files.next(
      new Array(filesCount)
        .fill(0)
        .map((v, i) => new DataFile(i, undefined, undefined))
    );
  ...
  private processFile(messageData: ArrayBuffer): void {
    ...
    this._files.value[index] = new DataFile(
      index,
  ...
  private processRemoveFile(messageData: ArrayBuffer): void {
    const fileName = ArrayUtilsService.bufferToString(
