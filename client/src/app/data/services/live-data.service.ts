...
export class LiveDataService extends BoardMessagingBaseService {
  ...
  private sendSourcesCount(): void {
    this.sendCommand(
      MonitoringCommands.SourcesCount,
      new Uint8Array([+autoSubscribe])
    );
  }
  ...
  private sendPutExtra(index: number, extra: readonly number[]): void {
    this.sendCommand(
      MonitoringCommands.PutExtra,
      new Uint8Array([index, extra.length].concat(extra))
    );
  }
  ...
  private processSourcesCount(messageData: ArrayBuffer): void {
    const sourceIndexes = new Uint8Array(
      new BufferReader(messageData).sliceSized()
    );
  ...
  private processSourceData(messageData: ArrayBuffer): void {
    ...
    for (let sourceIndex = 0; sourceIndex < sourcesDataLength; sourceIndex++) {
      ...
      sourcesData[sourceIndex] = new SourceData(
        indexFlags & SourceFlags.ClearMask,
        // tslint:disable-next-line: no-bitwise
