...
void loop()
{
	...
	if (systemState != SYSTEM_STATE_CAPTURING &&
			systemState != SYSTEM_STATE_RESTARTING &&
			Serial.available())
	{
		handleSerial();
	}
...
void sendSystemState(uint8_t number)
{
	...
	sendBin(number, sizeof(messageOut->subject) +
											sizeof(messageOut->command) +
											sizeof(uint8_t) +
											sizeof(uint8_t) +
											sizeof(timestamp));
}
...
void setSystemState(uint8_t state)
{
	...
	if (state == SYSTEM_STATE_CAPTURING)
	{
		...
		for (uint8_t deviceIndex = 0; deviceIndex < deviceCount; deviceIndex++)
		{
			...
			if (!(device->flags & DEVICE_FLAG_SYSTEM) &&
					(device->flags & DEVICE_FLAG_OUTPUT) &&
					device->type == DEVICE_TYPE_PIN)
			{
...
void setupWebServer()
{
	...
	webServer.on(
			"/upload", HTTP_POST, []() { webServer.send(200); }, handleWebServerUpload);

	webServer.on("/data-file", HTTP_GET, []() {
		if (systemState != SYSTEM_STATE_IDLE ||
				!webServer.hasArg("name") ||
				!strlen(webServer.arg("name").c_str()))
		{
			webServer.send(400);
		}
...
{
	if (subject >= sizeof(subscribers) / sizeof(subscribers_t) ||
			number >= sizeof(subscribers_t))
	{
		return;
	}
...
void handleMonitoringSubject(const message_in_t *message)
{
	...
	case MSG_CMD_MONITORING_SOURCES_COUNT:
		...
		for (deviceIndex = 0; deviceIndex < deviceCount; deviceIndex++)
		{
			if (devices[deviceIndex]->flags & DEVICE_FLAG_ENABLED &&
					!(devices[deviceIndex]->flags & DEVICE_FLAG_NO_DATA))
			{
...
void registerDevice(const uint8_t *data, uint8_t length)
{
	...
	for (extraBlock = 0; extraBlock < deviceCount; extraBlock++)
	{
		...
		for (uint8_t deviceIndex = 0; deviceIndex < deviceCount; deviceIndex++)
		{
			if (!(devices[deviceIndex]->flags & DEVICE_FLAG_NO_DATA) &&
					devices[deviceIndex]->extraBlock == extraBlock)
			{
				extraBlockInUse = 1;
				break;
			}
...
void readInputs()
{
	...
	for (uint8_t deviceIndex = 0; deviceIndex < deviceCount; deviceIndex++)
	{
		...
		if ((device->type == DEVICE_TYPE_DS18B20) &&
				(now - ds18b20Requested > ds18b20->millisToWaitForConversion(DS18B20_RESOLUTION)))
		{
